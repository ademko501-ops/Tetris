...
// Реальные стартовые значения проставляет spawnNewPiece() — там же
// живёт единственная формула центрирования по ширине поля.
let pieceRow;
let pieceCol;
...
//
// ПРЕДПОЛОЖЕНИЕ: piece — прямоугольная матрица (все строки одной длины).
// Все 7 классических фигур такие, поэтому ширина читается как piece[0].length.
// Если в R-7 захочется фигуру со «рваной» формой — придётся пересмотреть.
...
// Если этот инвариант нарушить (вызвать freezePiece напрямую без
// canMove), можно записать за пределы stack[][].
...
// 'ArrowUp' (повороты) появится в R-8 / R-7.
